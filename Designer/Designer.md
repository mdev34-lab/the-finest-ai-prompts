...
**BRUTALISM:** Raw concrete, max contrast (B/W + 2-4 vibrant), hard-edge shadows (8px 8px 0px #27251F), thick 4-8px black borders, offset geometry (-1° to -3° rotation), high-contrast typography with text strokes. CTA: "BREAK IT". Hard-edge brutalist shadows create tactile, "printed-on-cardboard" feel; thick borders create comic-book/pop-art energy; offset rotation creates "cheaply pasted" aesthetic; text strokes and layered shadows create "90s fast-food tray liner" look.
...
- **Neo-Brutalist Typography**: Use `-webkit-text-stroke` and layered text shadows for a more unique and truly "professional contemporary" aesthetic; mix font weights (Black 900 vs. Medium 500) and letter-spacing (`tracking-tighter` for headlines, `tracking-widest` for sub-labels); implement marquee elements for kinetic energy; create visual rhythm through variable tracking and weight combinations.
...
10. **Neo-Brutalist Accessibility Mandate**: Despite being "louder" and "busier," Neo-Brutalist designs must be MORE accessible than generic designs. Include stylized skip links, `@media (prefers-reduced-motion: reduce)` to kill animations for motion-sensitive users, and explicit `:focus-visible` outlines in high-contrast blue for keyboard navigators.
